...
void Log::init()
{
    try
    {
        ...
        s_learn_logger  = std::make_shared<spdlog::logger>("Net Learn", begin(default_log_sinks), end(default_log_sinks));
        s_hyper_logger  = std::make_shared<spdlog::logger>("Hyper Surfer", begin(default_log_sinks), end(default_log_sinks));
